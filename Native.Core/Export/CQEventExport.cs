...
namespace Native.App.Export
{
	...
	{	
		...
		{	
			return "9,cn.mikualpha.pcr.guild";	
		}	
		...
		{	
			...
			Type appDataType = typeof (AppData);	
			// 注册一个 CQApi 实例	
			AppInfo appInfo = new AppInfo ("cn.mikualpha.pcr.guild", 1, 9, "PCR公会战伤害计算工具", "1.0.0", 1, "初音未来Alpha", "PCR公会战伤害计算工具", authCode);	
			appDataType.GetRuntimeProperty ("CQApi").GetSetMethod (true).Invoke (null, new object[] { new CQApi (appInfo) });	
			AppData.UnityContainer.RegisterInstance<CQApi> ("cn.mikualpha.pcr.guild", AppData.CQApi);	
			// 向容器注册一个 CQLog 实例	
			appDataType.GetRuntimeProperty ("CQLog").GetSetMethod (true).Invoke (null, new object[] { new CQLog (authCode) });	
			AppData.UnityContainer.RegisterInstance<CQLog> ("cn.mikualpha.pcr.guild", AppData.CQLog);	
